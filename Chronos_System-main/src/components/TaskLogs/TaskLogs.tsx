...
const TaskLogs: React.FC<TaskLogsProps> = ({ currentUser }) => {
  ...
  const filteredLogs = mockTaskLogs.filter(log => {
    ...
    return matchesSearch && matchesEngineer && matchesProject && matchesStatus && matchesDepartment && matchesDate;
  });
  ...
  const sortedLogs = [...filteredLogs].sort((a, b) => {
    ...
    switch (sortBy) {
      ...
      case 'hours':
        ...
        break;
      case 'engineer':
        aValue = a.engineerName.toLowerCase();
        bValue = b.engineerName.toLowerCase();
        break;
      case 'project':
      ...
      case 'priority':
        ...
        break;
      case 'status':
        const statusOrder = { 'COMPLETED': 3, 'ONGOING': 2, 'PENDING': 1 };
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200 mt-4">
          <div className="text-sm text-gray-600">
            {sortedLogs.length} of {mockTaskLogs.length} logs
          </div>
        </div>
      </div>
