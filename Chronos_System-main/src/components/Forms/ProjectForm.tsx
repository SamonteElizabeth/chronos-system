...
interface ProjectFormProps {
  ...
  editProject?: any; // Project to edit
  isEditMode?: boolean;
}

const ProjectForm: React.FC<ProjectFormProps> = ({ isOpen, onClose, onSubmit, editProject, isEditMode = false }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.salesOrderNo.trim() && !/^\d{6}$/.test(formData.salesOrderNo.trim())) {
      newErrors.salesOrderNo = 'Sales Order No. must be exactly 6 digits';
    }
    ...
    if (!Number.isInteger(formData.sowMandays)) newErrors.sowMandays = 'SOW Mandays must be a whole number';
    if (formData.selectedSolutions.length === 0) newErrors.solution = 'At least one solution is required';
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Enter 6-digit Sales Order No."
                  maxLength={6}
                  pattern="[0-9]*"
                />
                {errors.salesOrderNo && (
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.sscfReferenceNo}
                  onChange={(e) => handleInputChange('sscfReferenceNo', e.target.value)}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors"
                  placeholder="Enter SSCF Reference No."
                  maxLength={50}
                />
              </div>
              ...
              {/* Solutions (Searchable Multiple Selection) */}
              <div className="lg:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Solutions (Multiple Selection) *
                </label>
                <div className="relative">
                  <input
                    type="text"
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors mb-2"
                    onChange={(e) => setSolutionSearch(e.target.value)}
                  />
                  <div className="border border-gray-300 rounded-lg p-3 max-h-40 overflow-y-auto">
                    {solutions
                      .filter(solution => 
                        solution.toLowerCase().includes(solutionSearch.toLowerCase())
                      )
                      .map((solution) => (
                      <label key={solution} className="flex items-center space-x-2 py-1">
                        <input
                          type="checkbox"
                          checked={formData.selectedSolutions.includes(solution)}
                          onChange={(e) => {
                            const newSelectedSolutions = e.target.checked
                              ? [...formData.selectedSolutions, solution]
                              : formData.selectedSolutions.filter(s => s !== solution);
                            handleInputChange('selectedSolutions', newSelectedSolutions);
                          }}
                  ...
                  {formData.selectedSolutions.length > 0 && (
                    <div className="mt-2">
                      <p className="text-sm text-gray-600 mb-1">Selected Solutions:</p>
                      <div className="flex flex-wrap gap-1">
                        {formData.selectedSolutions.map((solution) => (
                          <span
                            key={solution}
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const newSelectedSolutions = formData.selectedSolutions.filter(s => s !== solution);
                                handleInputChange('selectedSolutions', newSelectedSolutions);
                              }}
