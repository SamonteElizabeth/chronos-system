...
const ProjectGrid: React.FC<ProjectGridProps> = ({ currentUser }) => {
  ...
  const handleHoldProject = (project: Project) => {
    const updatedProjects = projects.map(p => 
      p.id === project.id 
        ? { ...p, status: p.status === 'ON_HOLD' ? 'ACTIVE' : 'ON_HOLD' }
        : p
    );
  ...
  return (
    <div className="space-y-6">
                    ...
                    {canAccess && (
                      <div className="flex items-center space-x-1">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleHoldProject(project);
                          }}
                          className="p-1 text-gray-400 hover:text-orange-600 transition-colors"
                          title={project.status === 'ON_HOLD' ? 'Resume Project' : 'Hold Project'}
                        >
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredProjects.map((project) => {
                    ...
                    return (
                      <tr key={project.id} className={`hover:bg-gray-50 ${isOverdue ? 'bg-red-50' : ''}`}>
      ...
      <ProjectForm
        ...
        editProject={isEditMode ? selectedProject : undefined}
