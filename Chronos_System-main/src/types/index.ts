export interface User {
  ...
  name: string;
  email: string;
  role: 'TASS' | 'PMO' | 'PM' | 'TM' | 'ENGINEER' | 'PM_DEPT_HEAD' | 'TM_DEPT_HEAD' | 'EEM' | 'EXECUTIVE';
  avatar?: string;
...
export interface ProjectFormData {
  ...
  assignedPM: string;
  assignedTM: string;
  sscfReferenceNo: string;
  selectedSolutions: string[];
}

export interface TaskFormData {
  taskName: string;
  subTask?: string; // Made optional
  startDate: string;
  dueDate: string;
  priority: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
  assignedEngineers: string[];
  ...
  description: string;
  projectId: string;
  subTasks?: SubTaskFormData[]; // Made optional
}
